...
If you prefer to use **other software, please contact support@ukllc.ac.uk** and the UK LLC Data Team will coordinate review of your request. Please be aware that alternative software packages may incur a fee. 
...
To **browse the data** in a specific view, right click on the view name and ‘**Select Top 1000 Rows**’. This will produce a grid-view of the first 1000 rows of data and a query window where data can be queried using SQL commands. This also works for tables.
### Video guide
...
3. Open '**r_label_helper.R**' with **RStudio** and find the comment '**ACTION1: enter/update project number...**'. Update the **proj_no variable** to your project number in the form '**LLC_0000**'  
...
2. One **LPS/data source** at a time e.g. MCS.  
3. **All tables** at once (not recommended due to performance issues).
...
2.	As stated in the comments, this method is not recommended if you have a large number of dataset because these will be slow to retrieve and add labels to.  
...
## 4. Stata
### Using the helper to retrieve and label data
...
3. When asked, input your project number in the example form '0001'.
...
6.	You are now ready to execute code. In cell 1 type:
*import stata_setup*
...
7.	You can then load a dataset and begin analyses. In each cell you intend to run stata code the first line must begin with *%%stata*. See the example below:  
