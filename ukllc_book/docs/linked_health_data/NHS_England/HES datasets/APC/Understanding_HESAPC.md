...
3) It uses standardised [coding systems](#7-coding-systems-used), enabling comparative work to be undertaken internationally
...
2) The [complex structure](#6-structure-of-the-dataset) of the dataset means researchers may have to undertake a substantial amount of data cleaning before starting their analyses
...
More information about HESAPC episodes and spells is in the [tips for researchers](#11-tips-for-researchers-using-hesapc-in-the-uk-llc-tre) below.  

### b) Recording of diagnoses and procedures  
Each Finished Consultant Episode (see [1)](#1-admissions-and-periods-of-care) above) can contain up to 20 diagnoses, and up to 24 procedures. See [Coding systems](#7-coding-systems-used) below for more information.
...
HESAPC uses two main medical coding systems: **ICD-10**<sup>1</sup> for diagnoses and **OPCS** for procedures and operations. As in all HES datasets, **NHS National Codes** are used for administrative information such as source of admission and discharge destination.  
...
* **Missing data** 
The amount of missing data varies widely between variables and across datasets. Throughout 2026, we will update this section with information about missingness in HESAPC.


## 11. Tips for researchers using HESAPC in the UK LLC TRE
### a) Working with episodes and spells
**Continuous Inpatient (CIP) spells**
In addition to the FCEs outlined [above](#6-structure-of-the-dataset), there is a more complex scenario again in which a patient is transferred to a different hospital during a single admission. In this instance, a new spell begins. To identify and measure continuous hospital stays, which include transfers to other hospitals, Continuous Inpatient (CIP) spells need to be derived (see figure 2).
...
>When applying to access linked HESAPC data in the UK LLC TRE, researchers must [**submit a codelist**](../../Coding/codelists.md) specifying the ICD-10 and / or OPCS codes which are relevant to their research question. 
...
**Payment by Results and changing NHSE priorities** also have implications for which conditions are recorded. See [Further Reading](#13-further-reading) below for more information on how NHS costings have changed over time.
...
## 13. Further reading  
Amies-Cull B, Luengo-Fernandez R, Scarborough, P et al. **NHS reference costs: a history 
and cautionary note.** Health Economics Review 2023 (13, 54). https://doi.org/10.1186/s13561-023-00469-0  
